...
export function HomeFeed({
  ...
  const totalValue = userHistory.reduce(
    (acc, item) => acc + (item.priceRange.high + item.priceRange.low) / 2,
    0
  );
          ...
          <div className="py-12 text-center">
            {activeTab === "discover" ? (
              ...
              <>
                ...
                <p className="text-slate-400 text-xs mt-1">
                  Tap the + button to capture your first!
                </p>
              </>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-1 sm:gap-2">
            {activeTab === "discover"
              ? publicTreasures.map((treasure) => {
                  const avgValue =
                    (treasure.price_low + treasure.price_high) / 2;
                  return (
                    <Link
                      key={treasure.id}
              ...
              : userHistory.map((item) => {
                  const avgValue =
                    (item.priceRange.low + item.priceRange.high) / 2;
                  return (
                    <Link
                      key={item.id}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-grow min-w-0">
                        ...
                        <p className="text-xs text-slate-500">
                          {item.category} {item.era ? `· ${item.era}` : ""}
                        </p>
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-sm font-bold text-teal-600">
                            {formatCurrency(avgValue, item.currency)}
