...
type AppState = 'landing' | 'form' | 'uploading' | 'loading' | 'result' | 'accepted' | 'declined';
...
export default function BullseyePage() {
  ...
  // When a file is captured, immediately compress + upload to storage
  const handleCapture = useCallback(async (fileList: FileList | null) => {
    if (!fileList || fileList.length === 0) return;

    // Move to form state if on landing
  ...
  const submit = async () => {
    if (uploadedUrls.length === 0) { setError('Please add at least one photo'); return; }
    setError(null);
  ...
  // --- File input handler (shared by both inputs) ---
  ...
  return (
    <div className="min-h-screen">
      {/* Hidden file inputs — always mounted so onChange survives camera */}
      <input
        id="camera-input"
        type="file"
        accept="image/*"
      ...
      {state === 'form' && (
        <div className="min-h-screen px-4 py-8 max-w-lg mx-auto">
          ...
          {/* Mobile: opens iOS action sheet (Take Photo / Photo Library) */}
          ...
          {/* Image previews — from uploaded URLs (not File objects) */}
          {uploadedUrls.length > 0 && (
            <div className="mt-4">
              <div className="text-xs text-slate-400 mb-2">{uploadedUrls.length} of 5 photos</div>
          ...
          <button
            onClick={submit}
            disabled={uploadedUrls.length === 0 || isSubmitting || isUploading}
